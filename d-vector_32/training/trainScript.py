...
def compute_mfcc_arduino_style(file_path):
    ...
    # Manually implement reflection padding
    ...
    mel_f = librosa.filters.mel(sr=SR, n_fft=n_fft, n_mels=40, 
                                htk=True, norm=1)  # Important: norm=1 for HTK style
...
def build_dvector_model(input_shape=(32, 13, 1), embedding_dim=32):
...
#Export the .tflite and .cc file for Arduino---------------------------------------------------
def representative_dataset():
    ...
    for fpath in files[:100]:  # use up to 100 samples
        y, sr = librosa.load(fpath, sr=SR, duration=1.0)
        if len(y) < SR:
        ...
        mfcc = mfcc.T.astype(np.float32)  # shape: (32, 13)
        yield [np.expand_dims(mfcc, axis=0)]  # shape: (1, 32, 13)
