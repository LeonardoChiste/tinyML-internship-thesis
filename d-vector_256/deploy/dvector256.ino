...
constexpr int TENSOR_ARENA_SIZE = 85 * 1024; // ~80KB
...
float cosineSimilarity(const int8_t* vecA, const int8_t* vecB, int size) {
  int32_t dot = 0, normA = 0, normB = 0;
  ...
  return (float) dot / (sqrt((float)normA) * sqrt((float)normB));
}
...
void setup() {
  ...
  PDM.setGain(80); //Gain can influence the inferencing, if too high it does not work
...
void loop() {
  ...
  if (audioReady) {
    ...
    int8_t* current_d_vector = output->data.int8;
    ...
    if (sim_me > THRESHOLD_UNKNOWN) digitalWrite(LEDR, LOW);  //blink, until next audio is captured
